...
const information = function (req, res) {
	const theImg = (req.params.image) ? desPath + req.params.image : srcImage;
	im.identify(theImg, function (err, features) {
		if (err) throw err;
		res.json({ "images_data": features });
	});
}

app.get('/getimage/information', information);
app.get('/getimage/information/:image', information);
